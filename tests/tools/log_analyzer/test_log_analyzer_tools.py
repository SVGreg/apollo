...
async def test_spawn_log_reader(apollo_context, mock_state):
    """Test the spawn_log_reader tool exposed by LogAnalyzerNode."""
    from apollo.sdk.utils import load_llm_config_override
    from pathlib import Path

    node = LogAnalyzerNode(ctx=apollo_context)
    apollo_context.llm_config = load_llm_config_override(Path("llm-config.json"))
    if apollo_context.data_engine:
        apollo_context.data_engine.start_session("test_session")
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_log_analyzer_run(apollo_context, mock_state):
    ...
    if apollo_context.data_engine:
        apollo_context.data_engine.start_session("test_log_analyzer_session")
