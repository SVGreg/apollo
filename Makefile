...
smoke-sim: ## Boot an iPhone simulator, run the driver smoke and a fake-LLM Flash task on it (no API key)
	...
	APOLLO_SIM_UDID=$$udid GOOGLE_API_KEY="$${GOOGLE_API_KEY:-test-placeholder}" uv run pytest tests/integration/test_ios_simulator_smoke.py tests/integration/test_sdk_ios_simulator.py -m ios_sim -q -o faulthandler_timeout=900 && \
