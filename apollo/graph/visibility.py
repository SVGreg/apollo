...
``extra="forbid"`` on :class:`~apollo.graph.state.State` catches ghost
...
unchanged unless ``APOLLO_STRICT_STATE=1`` (set in tests), in which case field
...
def strict_enabled() -> bool:
    return os.environ.get("APOLLO_STRICT_STATE", "") == "1"


class StateView:
    """Read-policing proxy handed to nodes under ``APOLLO_STRICT_STATE=1``.
