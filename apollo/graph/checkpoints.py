...
from apollo.utils.notes import append_note_content, get_note_file_path
...
def _finding_registry(ctx) -> dict:
    ...
    Kept as a dynamic context attribute (``ApolloContext`` allows extras) so
...
async def spawn_pending_checkpoints(ctx, state, anchor_step_id: str | None) -> list[str]:
    ...
    from apollo.agents.checker.checker import run_checkpoint_check
