...
The standard readiness probes answer "can Apollo drive a phone on this
machine?". This probe answers the question that precedes it when Apollo is
...
``mobile_diagnose`` MCP tool and ``apollo doctor`` run it explicitly.
...
#: Client names accepted by ``apollo mcp --install`` (see interfaces/cli/commands/mcp.py).
    ...
    if client:
        return [
            ProbeAction(
                ...
                payload=f"uv run apollo mcp --install {client}",
            )
        ]
    return [
        ProbeAction(
            ...
            payload=(
                ...
                " they are using, then run `uv run apollo mcp --install <client>` with"
...
def directory_is_writable(path: Path) -> tuple[bool, str | None]:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(dir=path, prefix=".apollo-diag-", delete=True):
            pass
        return True, None
...
def intended_traces_path() -> Path:
    ...
    Mirrors the selection order of :func:`apollo.config.paths.get_default_traces_path`
    (``APOLLO_TRACES_DIR`` env, else the user app dir when the install mode
    ...
    """
    from apollo.config import paths as apollo_paths

    env_traces = os.getenv(apollo_paths.ENV_APOLLO_TRACES_DIR)
    if env_traces:
        return Path(env_traces)
    if apollo_paths._use_user_app_dir():
        return apollo_paths.get_app_dir() / "traces"
    return Path(apollo_paths.ROOT_DIR) / "traces"
...
def resolve_traces_dir() -> tuple[Path, str | None]:
    ...
    """
    from apollo.config.paths import get_default_traces_path
...
class IntegrationHostProbe(BaseProbe):
    """Probe verifying the MCP / IDE host process can actually run Apollo tasks."""
    ...
    async def probe(self) -> ProbeResult:
        ...
        from apollo.config.paths import ROOT_DIR, get_env_file, is_source_checkout
        ...
        from apollo.runtime.server_lifecycle import is_port_in_use
        ...
        daemon_reachable = False if standalone else is_apollo_daemon()
            ...
            actions.append(
                ProbeAction(
                    ...
                    payload=(
                        ...
                        "APOLLO_TRACES_DIR at a writable directory in the MCP server's env block, "
        ...
        if port_hijacked:
            ...
            problems.append(
                f"port {DEFAULT_DAEMON_PORT} is held by a process that is not the Apollo daemon"
            )
            actions.append(
                ProbeAction(
                    action_type="hint",
                    ...
                    payload=(
                        ...
                        " (often a dev server). Stop it, or set APOLLO_DAEMON_PORT to a free port in"
            ...
            actions.append(
                ProbeAction(
                    ...
                    payload=(
                        ...
                        " server's env block; the file is created by `apollo init`."
                    ),
                )
            )
        ...
        if not daemon_reachable and not port_hijacked:
            actions.append(
                ProbeAction(
                    ...
                    payload=(
                        "Standalone mode is forced (APOLLO_STANDALONE=1); tasks run as detached"
                        " runner processes."
                        if standalone
                        else "The Apollo daemon is not running; mobile_run_task starts it on the"
