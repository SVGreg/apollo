    ...
    async def probe(self) -> ProbeResult:
        ...
        if is_supported:
            ...
            return ProbeResult(
                ...
                description=f"Python {py_ver_str} detected. Apollo requires Python >= 3.12.",
                metadata=metadata,
                actions=[
                    ProbeAction(
                        action_type="link",
...
class SystemConfigProbe(BaseProbe):
    """Probe verifying config/apollo.jsonc and settings integrity."""
    ...
    async def probe(self) -> ProbeResult:
        try:
            ...
            metadata = {
                "valid": True,
                "config_file": "config/apollo.jsonc",
                "planner_model": planner_model,
            }
            return ProbeResult(
                id=self.probe_id,
                category=self.category,
                title="Apollo System Configuration",
                status=ProbeStatus.PASS,
                is_blocker=self.is_blocker,
                ...
                actions=[
                    ProbeAction(
                        ...
                        payload="config/apollo.jsonc is loaded and valid.",
                    )
                ],
            )
            ...
            return ProbeResult(
                ...
                description=f"Failed to parse config/apollo.jsonc: {e}",
                metadata=metadata,
                actions=[
                    ProbeAction(
                        action_type="command",
                        label="Run Apollo Init",
                        payload="apollo init",
                    )
                ],
            )
