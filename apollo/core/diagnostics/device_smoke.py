...
The most common "device shows Connected but APOLLO cannot observe it" failure
...
the cached device controller through ``apollo.mcp.adb_server._get_controller``
...
* ``u2.connect`` restarts the on-device UIAutomator server when it believes the
  server is unhealthy. Running that against a device another APOLLO process is
...
def _uiautomator_fix(serial: str | None) -> list[str]:
    ...
    return [
        ...
        (
            ...
            "(run with the APOLLO virtualenv Python), or reboot the device."
        ),
    ]
...
def _resolve_requested_serial(device_serial: str | None) -> str | None:
    ...
    return (
        device_serial or os.environ.get("APOLLO_DEVICE_ID") or os.environ.get("ADB_DEVICE_SERIAL")
    )


def _find_busy_owner(target_serial: str | None) -> tuple[str, Any] | None:
    """Return ``(device_key, owner)`` when a live APOLLO process owns the target device.
...
async def _run_in_daemon_thread(fn: Callable[[], T], timeout: float, what: str) -> T:
    ...
    threading.Thread(target=_worker, name=f"apollo-smoke-{what}", daemon=True).start()
    try:
...
async def smoke_test_device(
    ...
    from apollo.clients.screen_client_factory import describe_backend
