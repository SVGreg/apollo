...
from apollo.config.paths import ROOT_DIR, get_app_dir
...
class AdbServerConnectionManager:
    ...
    async def connect(
        ...
        *,
        ...
        if persist and not persisted:
            message = (
                f"{message} The connection is active for this session, but Apollo could not "
    ...
    async def use_local_server(self, *, persist: bool = True) -> dict[str, Any]:
        ...
        message = "Apollo is now using the local ADB server."
        if persist and not persisted:
