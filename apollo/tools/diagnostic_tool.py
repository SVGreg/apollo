...
async def _run_diagnoser_logic(
    ...
    if ctx is None:
        return ToolFailure("Error: ApolloContext is required for Diagnoser.")
    agent = Diagnoser(ctx)
    try:
...
class AskDiagnoserTool(ApolloTool):
...
def get_ask_diagnostic_agent_tool(ctx: ApolloContext) -> BaseTool:
    """Exports ask_diagnoser as a LangChain BaseTool."""
    return trace_langchain_tool(ask_diagnoser.to_langchain_tool(ctx), ctx)


def get_ask_diagnoser_tool(ctx: ApolloContext) -> BaseTool:
