...
One search surface over a :class:`~apollo.data_engine.history_reader.HistoryReader`'s
...
def _step_label(step: dict, session_start: float | None) -> str:
    from apollo.memory.chunking import step_offset_label
...
def search_history_text(
    ...
    *,
    ...
    if range_start is not None and scoped_steps:
        from apollo.memory.chunking import build_action_ledger
    ...
    if terms:
        ...
        try:
            from apollo.utils.notes import list_notes_keys, read_note_content
