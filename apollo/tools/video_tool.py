...
class VideoAnalyzerTool(ApolloTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if ctx is None:
                raise ValueError("ApolloContext is required for VideoAnalyzer.")
...
class VideoAnalyzerPureTool(ApolloTool):
...
def get_video_analyzer_tool(
    ctx: ApolloContext, role: Literal["operator", "diagnoser"] = "operator"
) -> BaseTool:
...
def get_video_analyzer_tool_pure(ctx: ApolloContext) -> BaseTool:
