...
"""Universal ADB command and background task management tools for APOLLO.

State model
-----------
Every automation task owns one :class:`AdbTaskRegistry` (stored on its
:class:`~apollo.context.ApolloContext`).  The registry holds the running
...
from apollo.agents.log_analyzer.output_analyzer import TaskOutputAnalyzerNode
...
class AdbTaskRegistry:
    ...
    async def shutdown(self, ctx: ApolloContext | None = None) -> int:
...
def get_adb_task_registry(ctx: ApolloContext | None) -> AdbTaskRegistry:
...
async def shutdown_adb_background_tasks(ctx: ApolloContext | None) -> int:
...
def _get_task_info(task_id: str, ctx: ApolloContext | None) -> dict[str, Any] | None:
...
class BackgroundTask:
    ...
    async def start(self, ctx: ApolloContext):
        """Starts the background output listener task."""
        self.listener_task = asyncio.create_task(self._listen(ctx))

    async def stop(self, ctx: ApolloContext | None, status: str = "killed") -> None:
    ...
    def _finalize(self, ctx: ApolloContext | None) -> None:
    ...
    async def _listen(self, ctx: ApolloContext):
...
def _build_phone_script(
    ...
    if run_persistent:
        script += (
            "_apollo_ec=$?\n"
            f'echo "{_EXIT_CODE_MARKER}$_apollo_ec"\n'
...
class RunAdbCommandTool(ApolloTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if os.environ.get("APOLLO_CLOUD_MODE") == "1" or (
...
def get_run_adb_command_tool(ctx: ApolloContext) -> BaseTool:
...
class ManageTaskTool(ApolloTool):
...
def get_manage_task_tool(ctx: ApolloContext) -> BaseTool:
...
class RunShortAdbCommandTool(ApolloTool):
...
def get_run_short_adb_command_tool(ctx: ApolloContext) -> BaseTool:
...
class AnalyzeTaskOutputTool(ApolloTool):
...
def get_analyze_task_output_tool(ctx: ApolloContext) -> BaseTool:
