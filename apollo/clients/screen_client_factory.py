...
Three backends, chosen by ``APOLLO_HIERARCHY_BACKEND`` (environment or
``.env``):
...
from apollo.clients.ui_automator_client import UIAutomatorClient, UIAutomatorScreenData
from apollo.config.constants import ENV_APOLLO_HIERARCHY_BACKEND
from apollo.runtime.adb_endpoint import adb_command
from apollo.runtime.helper_manager import ProvisionEvent
...
def resolve_backend(value: str | HierarchyBackend | None = None) -> HierarchyBackend:
    ...
    text = value
    if text is None:
        text = os.environ.get(ENV_APOLLO_HIERARCHY_BACKEND)
    if text is None:
        try:
            from apollo.config import settings

            text = getattr(settings, "APOLLO_HIERARCHY_BACKEND", None)
...
_BACKEND_LABEL = {"helper": "the Apollo accessibility helper", "uiautomator": "UIAutomator2"}
