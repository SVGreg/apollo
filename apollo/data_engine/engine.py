...
from apollo.config import PAUSE_FILE, get_ipc_port_file, read_ipc_port, settings
...
from apollo.utils.text import safe_extract_text
...
class DataEngine:
    ...
    def _ipc_port_candidates() -> list[int]:
        ...
        A worker inherits ``APOLLO_IPC_PORT`` when it starts. If the Windows UI
    ...
    def _publish(self, event_type: str, data: Any):
        ...
        if os.environ.get("APOLLO_CLOUD_MODE") == "1":
            try:
                ...
                sid = os.environ.get("APOLLO_CLOUD_SESSION_ID") or (
    ...
    def start_session(
        ...
        try:
            ...
            DeviceExecutionLock.annotate_active_owner(
                ...
                ingress=os.getenv("APOLLO_TASK_INGRESS") or "sdk",
            )
        except Exception as exc:
    ...
    def end_session(self, status: str = "completed"):
        ...
        try:
            from apollo.services.token_meter import log_session_summary
    ...
    def _run_in_background(self, fn, *args):
        ...
        try:
            ...
            thread = threading.Thread(target=run_and_cleanup, name="apollo-storage", daemon=True)
