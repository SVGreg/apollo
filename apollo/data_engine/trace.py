...
from apollo.data_engine.context_vars import (
    CURRENT_NODE_NAME,
    CURRENT_TRACE_ID,
)
...
def trace(
    ...
    Args:
        ...
        ctx: Optional ApolloContext. If not provided, tries to find it in
          arguments.
    ...
    def decorator(func):
        ...
        def _get_ctx(*args, **kwargs) -> ApolloContext | None:
            if ctx:
                return ctx
            # Try to find in args
            for arg in args:
                if isinstance(arg, ApolloContext):
                    return arg
                if hasattr(arg, "ctx") and isinstance(getattr(arg, "ctx"), ApolloContext):
                    return getattr(arg, "ctx")
            # Try to find in kwargs
            for v in kwargs.values():
                if isinstance(v, ApolloContext):
                    return v
                if hasattr(v, "ctx") and isinstance(getattr(v, "ctx"), ApolloContext):
...
def trace_langchain_tool(tool: BaseTool, ctx: ApolloContext) -> BaseTool:
