...
from apollo.constants import RECURSION_LIMIT
...
from apollo.runtime.cancel_requests import watch_for_cancel_request
...
from apollo.sdk.builders.agent_config_builder import get_default_agent_config
from apollo.sdk.builders.task_request_builder import TaskRequestBuilder
from apollo.sdk.types.agent import AgentConfig
...
from apollo.utils.app_launch_utils import _handle_initial_app_launch
...
def run_tuning_summary(config: AgentConfig, profile: str | None) -> dict[str, str] | None:
    ...
    sourced in ``apollo.config``. Returns ``None`` for Flash runs.
    """
...
class Agent:
    ...
    async def _init_internal(
        ...
        logger.info("✅ Apollo agent initialized.")
    ...
    async def _prewarm_llm_connections(self, api_key: str | None = None):
        ...
        if os.environ.get("APOLLO_FAKE_LLM") == "1":
            logger.info("APOLLO_FAKE_LLM=1 — skipping real LLM connection pre-warming.")
            publish_startup_progress(
    ...
    async def _run_task(
        ...
        task_id = str(
            self._session_id
            or os.getenv("APOLLO_SESSION_ID")
            or os.getenv("APOLLO_CLOUD_SESSION_ID")
            or uuid.uuid4()
        )
        ...
        async def _execute_task_logic():
            ...
            device_lock = (
                ...
                if already_held
                    ...
                    ingress=os.getenv("APOLLO_TASK_INGRESS") or "agent",
                )
            )
            try:
                if device_lock is not None and os.environ.get("APOLLO_CLOUD_MODE") != "1":
    ...
    async def _prepare_device_environment(self, context: ApolloContext):
    ...
    async def _prepare_app_lock(self, task: Task, context: ApolloContext):
    ...
    def _prepare_tracing(self, task: Task, context: ApolloContext):
    ...
    async def _finalize_tracing_safely(self, task: Task, context: ApolloContext):
    ...
    async def _finalize_tracing(self, task: Task, context: ApolloContext):
        if context.data_engine:
