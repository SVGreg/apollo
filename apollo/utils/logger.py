...
class DataEngineHandler(logging.Handler):
    ...
    def _drain_queue(self):
        while True:
            ...
            if item is None:
                break
            engine_mod = sys.modules.get("apollo.data_engine.engine")
...
class ApolloLogger:
    def __init__(
        ...
        if os.environ.get("APOLLO_MCP_SERVER") == "true":
            traces_dir = os.environ.get("APOLLO_TRACES_PATH", "traces")
...
def get_logger(
    ...
    Returns:
        ApolloLogger instance
    """
    if name not in _loggers:
        _loggers[name] = ApolloLogger(
            name=name,
