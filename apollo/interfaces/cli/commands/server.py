...
"""Server and cloud proxy commands (apollo server)."""
...
server_app.command("restart", help="Restart running Apollo Web UI & server.")(restart_command)
server_app.command("stop", help="Stop running Apollo Web UI & server.")(stop_command)
server_app.command("status", help="Display Apollo Web UI & server status.")(status_command)
...
class CloudRunProxyHandler(http.server.BaseHTTPRequestHandler):
    target_url: str = os.getenv("APOLLO_CLOUD_URL", "http://127.0.0.1:8000")
    ...
    def proxy_request(self, method: str):
        ...
        tenant_token = os.getenv("APOLLO_TENANT_TOKEN")
        if tenant_token:
            headers["X-Apollo-Token"] = tenant_token
...
def web_proxy_command(
    ...
    typer.secho(
        f"🚀 Starting Apollo Cloud Web Proxy at http://localhost:{port}", fg=typer.colors.GREEN
    )
