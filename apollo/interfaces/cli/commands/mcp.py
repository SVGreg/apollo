...
"""Model Context Protocol (MCP) server command (apollo mcp)."""
...
def _eager_tools_config() -> dict[str, dict[str, bool]]:
    ...
    return {tool_name: {"eager": True} for tool_name in APOLLO_MCP_TOOLS}
...
def _get_config_snippet(client: str, python_exe: str, project_root: str) -> dict:
    ...
    if client == "antigravity":
            ...
            "mcp_servers": {
                "apollo": {
                    **config_with_cwd,
                    ...
                    # explicit allow-list guarantees all Apollo tools are
                    # registered. The host may still present schemas through
                    # tool search to control context size.
                    "enabled": True,
                    "required": True,
                    "startup_timeout_sec": 120,
                    "enabled_tools": list(APOLLO_MCP_TOOLS),
                }
            }
        }
    elif client == "vscode":
        return {"servers": {"apollo": {**config_with_cwd, "type": "stdio"}}}
    elif client in ("claude", "claude_code", "claude_desktop"):
        return {"mcpServers": {"apollo": {**config_body, "type": "stdio"}}}
    elif client == "cline":
        return {
            "mcpServers": {
                "apollo": {
        ...
        return {"mcpServers": {"apollo": config_body}}
...
def _codex_toml_block(server_config: dict) -> str:
    ...
    if env:
        lines.extend(["", "[mcp_servers.apollo.env]"])
        for key, value in env.items():
            lines.append(f"{key} = {json.dumps(str(value), ensure_ascii=False)}")
    lines.append("# END APOLLO MCP CONFIG")
    return "\n".join(lines) + "\n"


def _merge_codex_toml(file_path: Path, server_config: dict) -> bool:
    """Adds or updates Apollo in Codex config.toml while preserving unrelated settings."""
    try:
        file_path.parent.mkdir(parents=True, exist_ok=True)
        existing = file_path.read_text(encoding="utf-8") if file_path.exists() else ""
        managed_block = _codex_toml_block(server_config)
        begin_marker = "# BEGIN APOLLO MCP CONFIG"
        end_marker = "# END APOLLO MCP CONFIG"
        ...
        if begin_marker in existing and end_marker in existing:
            ...
            # Remove an older, unmanaged Apollo table before adding the managed block.
            kept_lines: list[str] = []
            skipping_apollo = False
            ...
            for line in existing.splitlines(keepends=True):
                ...
                if header_match:
                    ...
                    skipping_apollo = normalized == "mcp_servers.apollo" or normalized.startswith(
                        "mcp_servers.apollo."
                    )
                if not skipping_apollo:
...
def _remove_rules_block(file_path: Path) -> bool:
    """Removes a previously injected managed APOLLO block from a rules file.
    ...
    try:
        if not file_path.exists():
            return True
        begin_marker = "<!-- BEGIN APOLLO MOBILE TESTING RULES -->"
        end_marker = "<!-- END APOLLO MOBILE TESTING RULES -->"
...
def _inject_rules_block(file_path: Path, content: str) -> bool:
    """Injects or updates a managed APOLLO block in a global rules file without overwriting user content."""
    try:
        file_path.parent.mkdir(parents=True, exist_ok=True)
        begin_marker = "<!-- BEGIN APOLLO MOBILE TESTING RULES -->"
        end_marker = "<!-- END APOLLO MOBILE TESTING RULES -->"
...
def _write_cursor_mdc(file_path: Path, content: str) -> bool:
    ...
    try:
        ...
        mdc_header = (
            "---\n"
            "description: Apollo Mobile Testing Mindset & Rules\n"
            "globs: **/*\n"
            "alwaysApply: true\n"
            "---\n\n"
        )
...
def install_rules(client: str, project_root: str) -> list[str]:
    """Auto-installs APOLLO testing rules globally into user IDE rule directories across any OS."""
    ...
    for target in targets:
        if target in ("antigravity", "jetski"):
            ...
            global_rule = Path.home() / ".gemini" / "rules" / "apollo.md"
            ...
            global_mdc = Path.home() / ".cursor" / "rules" / "apollo.mdc"
            ...
            global_rule = Path.home() / ".claude" / "rules" / "apollo.md"
            ...
            global_rule = Path.home() / ".codeium" / "windsurf" / "rules" / "apollo.md"
            ...
            global_rule = Path.home() / ".vscode" / "rules" / "apollo.md"
            user_rule = _get_vscode_user_dir() / "rules" / "apollo.md"
            if _write_rule_file(global_rule, raw_rules):
                installed_paths.append(str(global_rule))
            ...
            global_rule = Path.home() / ".cline" / "rules" / "apollo.md"
            ...
            global_rule = Path.home() / ".roo" / "rules" / "apollo.md"
            ...
            global_rule = Path.home() / ".openclaw" / "rules" / "apollo.md"
...
def install_mcp_config(client: str, python_exe: str, project_root: str) -> list[str]:
    """Auto-installs/merges APOLLO MCP configuration and testing rules into IDE config files across any OS."""
    installed_paths: list[str] = []
    targets = (
        [
            "antigravity",
    ...
    for target in targets:
        ...
        if target in ("antigravity", "jetski"):
            ...
            if _merge_json_file(jetski_path, "apollo", legacy_server_cfg):
                installed_paths.append(str(jetski_path))
            if _merge_json_file(antigravity_legacy_path, "apollo", current_server_cfg):
                installed_paths.append(str(antigravity_legacy_path))
            if _merge_json_file(config_path, "apollo", current_server_cfg):
                installed_paths.append(str(config_path))
        elif target in ("claude", "claude_code", "claude_desktop"):
            server_cfg = snippet["mcpServers"]["apollo"]
            if sys.platform == "darwin":
                claude_path = (
                    Path.home()
                    / "Library"
            ...
            if _merge_json_file(claude_path, "apollo", server_cfg):
            ...
            for cline_path in cline_paths:
                if _merge_json_file(cline_path, "apollo", server_cfg):
            ...
            for roo_path in roo_paths:
                if _merge_json_file(roo_path, "apollo", server_cfg):
                    installed_paths.append(str(roo_path))
        elif target == "openclaw":
            server_cfg = snippet["mcp"]["servers"]["apollo"]
            ...
            if _merge_json_file(
                ...
                remove_paths=(("plugins", "apollo_mcp"),),
            ):
                installed_paths.append(str(openclaw_path))
        elif target == "codex":
            server_cfg = snippet["mcp_servers"]["apollo"]
            codex_home = Path(os.environ.get("CODEX_HOME") or (Path.home() / ".codex"))
...
def mcp_command(
    ...
    install_config: Annotated[
        ...
        typer.Option(
            "--install",
            "-i",
            help="Auto-install and merge APOLLO MCP configuration and testing rules into 'antigravity', 'claude', 'cursor', 'windsurf', 'vscode', 'cline', 'roo', 'openclaw', 'codex', or 'all'.",
        ),
    ] = None,
    ...
    """Launch or configure the APOLLO Model Context Protocol (MCP) server."""
    ...
    if install_config:
        ...
        console.print(
            "[bold green]✔ Successfully installed APOLLO MCP server configuration & rules to:[/bold green]"
        )
        for path in installed_paths:
            console.print(f"  • [cyan]{path}[/cyan]")
        console.print(
            "\n[dim]Please restart or reload your IDE window to activate the Apollo MCP tools.[/dim]"
        )
        raise typer.Exit(0)
    ...
    try:
        ...
        if st in ("agent", "mobile", "apollo", "default"):
            logger.info(f"Starting Apollo Mobile Agent MCP Server over {transport}...")
            ...
            logger.info(f"Starting Apollo XML Fuzzy Search MCP Server over {transport}...")
            if transport.lower() == "sse":
