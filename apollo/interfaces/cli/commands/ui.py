...
"""UI Launch and Web Console CLI Command (apollo ui)."""
...
def running_via_console_script_shim() -> bool:
    ...
    (``Scripts/apollo.exe``) that stay resident for the lifetime of the CLI
    ...
    ``python -m apollo`` avoids the shim entirely.
    """
...
def warn_if_console_script_shim(console: Console) -> None:
    ...
    if running_via_console_script_shim():
        console.print(
            "   [dim]⚠ Running in foreground via the [bold]apollo.exe[/bold] launcher: package "
            "syncs (uv sync / uv run) cannot replace it while this server is running.\n"
            "     Prefer [bold]apollo restart[/bold] (background daemon) or launch with "
            "[bold]uv run python -m apollo ui[/bold] to avoid this.[/dim]\n"
        )
...
def load_session_reconciler() -> None:
    ...
    with ``apollo.runtime.server_lifecycle`` (inverted dependency: the base
...
def _poll_and_open_browser(url: str, stop_event: threading.Event, timeout: float = 15.0) -> None:
    ...
    while not stop_event.is_set() and (time.time() - start < timeout):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "Apollo-HealthCheck/1.0"})
...
def ui_command(
    ...
    """Launch the unified Apollo Showcase UI & Admin Console in one click."""
    console = Console()
    ...
    if restart:
        ...
        if sys.stdin.isatty():
            console.print(
                f"\n   [yellow]⚠ Port {port} is already in use by an active Apollo server{pid_str}.[/yellow]"
            )
            ...
            if choice == "2":
                console.print(
                    f"\n   [yellow]🔄 Terminating previous Apollo instance on port {port}...[/yellow]"
                )
                load_session_reconciler()
                stop_server(port=port, timeout=12.0)
            elif choice == "3":
            ...
            console.print(
                f"\n   [yellow]⚠ Port {port} is already in use by Apollo{pid_str}. "
                f"Use '[bold]apollo restart[/bold]' to restart.[/yellow]\n"
            )
            if open_browser:
                webbrowser.open(local_url)
            return
    ...
    console.print()
    msg = (
        f"[bold cyan]✨ Apollo Autonomous Mobile Agent UI[/bold cyan]\n\n"
    ...
    try:
        ...
        # drag the whole admin console into every `apollo <cmd>` invocation.
