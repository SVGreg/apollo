...
"""CLI commands for Apollo server lifecycle management: restart, stop, and status."""
...
from apollo.runtime.daemon_client import daemon_log_path, is_daemon_running, spawn_daemon
...
def restart_command(
    ...
    """Restart the Apollo Web UI server, terminating any existing instance on the port."""
    console.print()
    ...
    console.print(
        Panel(
            f"[bold cyan]🔄 Apollo Server Restart[/bold cyan]\n"
            f"[dim]Inspecting port {port} and recycling any running Apollo instances...[/dim]",
            border_style="cyan",
            expand=False,
        )
    )
    ...
    if existing_pids:
        ...
        console.print(
            f"   [yellow]🛑 Terminating running Apollo instance (PID: {pid_str})...[/yellow]"
        )
    else:
        console.print(f"   [dim]ℹ No active Apollo server found on port {port}.[/dim]")
    ...
    if daemon:
        ...
        console.print("   [cyan]🚀 Starting Apollo in detached background daemon mode...[/cyan]")
        ...
        if ready:
            console.print(
                f"   [bold green]✓ Apollo server running in background (PID: {proc.pid})[/bold green]\n"
                f"   🌐 [cyan]{local_url}[/cyan]\n"
                f"   🛠️  [cyan]{local_url}/admin[/cyan]\n"
                f"   📄 [dim]Logs: {log_path}[/dim]\n"
                f"   [dim]💡 Stop with [bold]apollo stop[/bold]; run in-terminal with [bold]apollo restart --foreground[/bold].[/dim]\n"
            )
            if open_browser:
    ...
    console.print(
        "   [cyan]🚀 Launching unified Apollo Showcase UI in current terminal...[/cyan]\n"
    )
...
def stop_command(
    port: Annotated[
        int,
        typer.Option("--port", "-p", help="Port of the Apollo server to stop."),
    ] = 8000,
    force: Annotated[
        bool,
        typer.Option(
    ...
    """Stop any running Apollo Web UI / server instance on the given port."""
    console.print()
    existing_pids = find_server_pids(port)
    if existing_pids:
        pid_str = ", ".join(map(str, existing_pids))
        console.print(
            f"   [yellow]🛑 Stopping Apollo server on port {port} (PID: {pid_str})...[/yellow]"
        )
    else:
    ...
    if stopped_pids:
        console.print(
            Panel(
                f"[bold green]✓ Apollo server stopped successfully.[/bold green]\n\n"
                f"[dim]{msg}[/dim]",
        ...
        console.print(
            Panel(
                f"[dim]No active Apollo server found on port {port}. Port is free.[/dim]",
                title="ℹ Status",
                border_style="dim",
                expand=False,
            )
...
def status_command(
    port: Annotated[
        int,
        typer.Option("--port", "-p", help="Port to query for Apollo server status."),
    ] = 8000,
) -> None:
    """Check whether the Apollo server is currently running and display runtime metadata."""
    console.print()
    ...
    if status["running"]:
        ...
        console.print(
            Panel(
                table,
                title="🚀 Apollo Server Status",
                border_style="green",
                expand=False,
            )
        )
        console.print(
            "   [dim]💡 To restart: [bold]apollo restart[/bold] | To stop: [bold]apollo stop[/bold][/dim]\n"
        )
    else:
        console.print(
            Panel(
                ...
                f"[dim]No active Apollo server process detected.[/dim]",
                title="ℹ Apollo Server Status",
                border_style="dim",
                expand=False,
            )
        )
        console.print(
            "   [dim]💡 To start: [bold]apollo ui[/bold] or [bold]./start.sh[/bold][/dim]\n"
        )
