...
"""System, environment, and device diagnostics (apollo doctor).

The command renders the same readiness probes the web console's device wizard
and the ``mobile_diagnose`` MCP tool run (:mod:`apollo.core.diagnostics`), so
...
from apollo.core.diagnostics.schema import ProbeResult, ProbeStatus
from apollo.platform import OSType, platform
...
def _render_footer(console: Console, verdict: Verdict, results: list[ProbeResult]) -> None:
    ...
    tips = [
        ...
        "",
        "• API keys & device setup: run [bold cyan]apollo init[/bold cyan].",
        "• Follow the [bold cyan]Run:[/bold cyan] lines above, then re-run "
        "[bold cyan]apollo doctor[/bold cyan].",
...
def doctor_command(
    ...
    if json_output:
        ...
        console.print(
            Panel(
                f"[bold cyan]☕ Apollo System & Environment Doctor[/bold cyan]\n"
