...
"""Task execution command (apollo run)."""
...
from apollo import Agent, Builders
from apollo.sdk.types.task import AgentProfile
from apollo.utils.cli_helpers import display_device_status
from apollo.utils.logger import get_logger
from apollo.utils.video import check_ffmpeg_available
import signal
...
async def execute_task(
    ...
    if effective_sid:
        os.environ["APOLLO_SESSION_ID"] = str(effective_sid)
    if not os.environ.get("APOLLO_TASK_INGRESS"):
        os.environ["APOLLO_TASK_INGRESS"] = "cli"
...
def run_command(
    ...
    standalone: Annotated[
        ...
        typer.Option(
            "--standalone",
            help="Run in standalone embedded mode without auto-spawning the Apollo Daemon.",
        ),
    ] = False,
) -> None:
    ...
    is_worker = (
        os.environ.get("APOLLO_TASK_WORKER") == "1"
        or os.environ.get("APOLLO_DEVICE_QUEUE_TICKET") is not None
    )
    is_standalone = standalone or os.environ.get("APOLLO_STANDALONE") == "1"

    # All platforms route through unified Apollo Daemon unless specifically configured as standalone
    if not is_worker and not is_standalone:
        try:
            ...
            console.print("[dim]Connecting to Apollo Daemon scheduler...[/dim]")
            ...
            if is_running and base_url:
                ...
                console.print(
                    f"[bold green]✓[/bold green] Apollo Daemon active at [cyan]{base_url}[/cyan]"
                )
                ...
                console.print(
                    "[yellow]Warning: Apollo Daemon could not be started. Falling back to local execution...[/yellow]"
                )
        except SystemExit:
            raise
        ...
        if "API_KEY" in err_msg or "requires" in err_msg:
            ...
            console.print(
                Panel(
                    ...
                    "💡 [bold cyan]Quick Fix:[/bold cyan] Run [bold green]apollo init[/bold green] to configure your API key in 10 seconds,\n"
