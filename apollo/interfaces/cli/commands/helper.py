...
"""``apollo helper``: manage the Accessibility Helper APK on attached devices.
...
helper_app = typer.Typer(help="Install, inspect, or remove the Apollo Accessibility Helper APK.")
...
def _idle_ready_serials() -> list[str]:
    """Every authorized device that no Apollo task currently holds."""
...
def helper_status(
    ...
    tunnel = {
        ...
        "shared": f"another Apollo process, host port {status['forward_port']}",
...
def helper_uninstall(
    ...
    target = _resolve_serial(serial)
    if not yes and not typer.confirm(f"Remove the Apollo accessibility helper from {target}?"):
...
def helper_parity(
    ...
    """
    from apollo.core.diagnostics.hierarchy_parity import compare_backends
