...
"""Dynamic Multi-Provider Model Router for APOLLO.
...
from apollo.llm.google.provider import supports_thinking_level
...
def _patch_langchain_google_genai():
    ...
    try:
        ...
        if getattr(ChatGoogleGenerativeAI, "_is_apollo_patched", False):
            return
        ...
        ChatGoogleGenerativeAI._is_apollo_patched = True
...
class ModelFactory:
    ...
    def create_model(cls, endpoint: ModelEndpoint) -> BaseChatModel:
        if os.environ.get("APOLLO_FAKE_LLM") == "1":
            from apollo.llm.fake_model import FakeChatModel

            delay = float(os.environ.get("APOLLO_FAKE_LLM_DELAY_S", "0") or 0)
            logger.warning(
                f"APOLLO_FAKE_LLM=1 — returning FakeChatModel (delay={delay}s) "
