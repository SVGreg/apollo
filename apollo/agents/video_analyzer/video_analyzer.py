...
``mock.patch("apollo.agents.video_analyzer.video_analyzer.<name>")``
...
from apollo.agents.video_analyzer.conflict_resolution import (
    ConflictResolutionService,
)
from apollo.agents.video_analyzer.blackboard import get_video_blackboard
from apollo.agents.video_analyzer.gemini_files import (
...
from apollo.agents.video_analyzer.universal_tools import (
    UNIVERSAL_MAIN_TOOLS,
    UNIVERSAL_SUBMIT_ANSWER_TOOL,
    build_native_tools_declaration,
    build_submit_answer_declaration,
)
from apollo.config import get_temp_dir, settings
from apollo.constants import SAFETY_SETTINGS_BLOCK_NONE
from apollo.context import ApolloContext
from apollo.controllers.controller_factory import get_controller
from apollo.data_engine.trace import CURRENT_TRACE_ID, TraceSpan, trace
from apollo.llm.google import (
    is_gemini_model,
    resolve_video_processing,
    strip_provider_prefix,
)
from apollo.llm.reliability import retry_policy_for
from apollo.services.llm import _record_llm_event, _record_llm_retry, get_llm
...
async def _invoke_with_retry(operation, label: str, max_attempts: int = 3):
    ...
    per-category policies in :mod:`apollo.llm.reliability` instead of ad-hoc
...
class VideoAnalyzer:
    ...
    async def _cleanup_universal_resources(self) -> None:
        ...
        if os.environ.get("KEEP_VIDEOS") or os.environ.get("APOLLO_DEBUG"):
            return
    ...
    async def upload_and_poll_file(self, compressed_path: Path) -> any:
        from apollo.agents.video_analyzer import gemini_files
    ...
    async def _exec_single_chunk(
        ...
        from apollo.agents.video_analyzer import chunk_native
    ...
    async def exec_analyze_audio_only(
        ...
        from apollo.agents.video_analyzer import audio_native
    ...
    async def _exec_single_chunk_universal(
        ...
        from apollo.agents.video_analyzer import universal_engine
    ...
    async def run(self, time_description: str, purpose: str) -> tuple[str, str]:
        ...
        from apollo.agents.video_analyzer import native_coordinator
