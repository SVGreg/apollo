...
from apollo.constants import CHECKER_MAX_ITERATIONS
...
from apollo.services.llm import acomplete, get_llm, invoke_llm_with_timeout_message
...
async def _execute_probe(ctx: ApolloContext, argv: list[str]) -> str:
...
def get_probe_tool(ctx: ApolloContext) -> BaseTool:
    @tool
...
def probes_enabled(ctx: ApolloContext) -> bool:
...
def build_checker_tools(
    ctx: ApolloContext, entry: Literal["checkpoint", "final"]
) -> list[BaseTool]:
...
def _load_steps(ctx: ApolloContext) -> list[dict] | str:
...
def _format_history(ctx: ApolloContext, limit: int = 60, steps: list[dict] | None = None) -> str:
    if steps is None:
...
def _format_user_guidance(ctx: ApolloContext, steps: list[dict] | None = None) -> str:
...
def _persist_stream(
    ...
    *,
    ...
    if not segments:
        return
    try:
        from apollo.graph.checkpoints import append_attempt_stream_record
...
def _announce(ctx: ApolloContext, **kwargs) -> None:
    """Lazy bridge to the checkpoint bus (avoids an import cycle at module load)."""
    try:
        from apollo.graph.checkpoints import announce_checker_attempt
...
async def _capture_final_screen(ctx: ApolloContext) -> tuple[str | None, str]:
