...
from apollo.tools.index import get_tool_by_name, get_tools_from_wrappers
...
def validate_plan_format(content: str) -> tuple[bool, str]:
    ...
    Single-sourced from :mod:`apollo.utils.plan_grammar`: any status character
...
def _check_gates(ctx: ApolloContext) -> tuple[bool, bool]:
...
def _checks_enabled(ctx: ApolloContext) -> bool:
...
class PlannerNode:
    def __init__(self, ctx: ApolloContext, tools: list[BaseTool] | None = None):
        self.ctx = ctx
        self.tools = tools or []
