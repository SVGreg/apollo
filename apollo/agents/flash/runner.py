...
"""Universal Multi-Model FlashRunner for Apollo.
...
from apollo.agents.flash.summarizer import VisualStepSummarizer, build_focus_context
from apollo.agents.operator.prompts import REASONING_REMINDER, UserGuidance, render_user_guidance
...
from apollo.data_engine.trace import trace
from apollo.graph.perception import _check_injected_instruction_file
from apollo.graph.state import State
from apollo.llm.structured import ParseFailure, parse_structured
from apollo.mcp.action_executor import McpActionExecutor
from apollo.mcp.observation import observe
from apollo.memory.transcript import PRO_UI_LIST_MARKER, TranscriptLedger, mark_ephemeral
...
from apollo.tools.history import history_tool_declarations
...
class FlashRunner:
    ...
    def __init__(self, ctx: ApolloContext, goal: str, max_turns: int | None = None):
        self.ctx = ctx
        self.goal = goal
        try:
    ...
    def _get_tools(self) -> list:
        ...
        if self._video_tools_enabled():
            from apollo.tools.video_tool import VIDEO_ANALYZER_TOOL
        ...
        if actuator is not None:
            from apollo.mcp.action_manifest import filter_declarations
    ...
    def _build_ledger(self) -> TranscriptLedger:
        ...
        if engine is not None:
            try:
                from apollo.memory import HistoryChunkManager
    ...
    async def run(self, state: State) -> dict:
        ...
        logger.info(
            f"Starting Apollo Flash reactive loop for goal: {self.goal}"
        ...
        while limit is None or turns < limit:
            turns += 1
            logger.info(f"--- Apollo Flash Turn {turns}{f'/{limit}' if limit else ''} ---")
