...
- :mod:`apollo.agents.validator.execution_loop` -- per-turn loop
  orchestration, screenshot/trace bookkeeping, failure-repair handling.
- :mod:`apollo.agents.validator.action_execution` -- decision parsing,
  turn-initial screenshot loading, single-action dispatch.
- :mod:`apollo.agents.validator.precondition_xml` -- XML-hierarchy safety
  net (element matching, coordinate self-healing, failure taxonomy).
- :mod:`apollo.agents.validator.precondition_pixel` -- pixel/VLM safety net.
...
from apollo.agents.validator import (
    action_execution,
    execution_loop,
    precondition_pixel,
    precondition_xml,
...
from apollo.constants import (  # noqa: F401  (patchable seams; the UI-change
...
from apollo.services.llm import acomplete_structured, get_llm  # noqa: F401
from apollo.utils.decorators import wrap_with_callbacks
...
class ValidatorNode:
    ...
    async def __call__(self, state: State):
        ...
        try:
            ...
            # In-process session teardown happens in ApolloContext.__aexit__; there
    ...
    async def _validate_action_precondition_pixel(
        ...
        patching ``apollo.agents.validator.validator.get_llm`` take effect.
        """
