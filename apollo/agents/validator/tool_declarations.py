...
"""Universal Tool Declarations and Execution Results for Apollo.

Execution itself lives in :class:`apollo.mcp.action_executor.McpActionExecutor`
...
from apollo.mcp.action_specs import tool_declaration
...
async def capture_screenshot_and_parse_ui(
    ...
    Thin adapter over :func:`apollo.mcp.observation.observe` that adds the LangGraph
...
# (apollo/mcp/action_specs.py); the historical constant names remain the public API.
...
# Same contract as the Operator's LangChain tool (apollo/tools/explorer_tool.py):
