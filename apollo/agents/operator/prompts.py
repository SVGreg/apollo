...
from apollo.tools.history import HISTORY_TOOL_NAMES
...
from apollo.utils.task_tree import SELF_DESCRIBED_MARKER, action_intent_phrase

logger = get_logger(__name__)


from apollo.agents.prompt_assembly import render_tool_enum, resolve_available
from apollo.mcp.action_specs import OPERATOR_SHELL_ORDER
...
# apollo/agents/prompt_assembly.py for the assembly rationale.
...
class PromptBuilder:
    def __init__(self):
        ...
        #: ``apollo.memory.transcript.EPHEMERAL_BLOCKS_KEY``).
...
class PromptComponent:
    async def __call__(self, builder: PromptBuilder, state: State, ctx: ApolloContext, **kwargs):
        raise NotImplementedError


def resolve_operator_prompt_tools(ctx: ApolloContext) -> frozenset[str]:
...
def _operator_grammar_flags(ctx: ApolloContext) -> tuple[bool, bool, bool]:
...
def _checkpoint_max_repairs(ctx: ApolloContext) -> int:
...
def _grammar_render_context(ctx: ApolloContext) -> dict:
...
def _legacy_elapsed_suffix(ctx: ApolloContext | None) -> str:
    """`` [T+mm:ss]`` for the legacy observation header (empty without a session clock)."""
    import time

    from apollo.memory.transcript import format_session_offset
...
class PlanRecitationPromptComponent(PromptComponent):
    ...
    async def __call__(self, builder: PromptBuilder, state: State, ctx: ApolloContext, **kwargs):
        from apollo.memory.transcript import PLAN_RECITATION_MARKER
...
def _session_offset_label(ctx: ApolloContext | None) -> str | None:
...
class HistoricalStateHintPromptComponent(PromptComponent):
    ...
    async def __call__(self, builder: PromptBuilder, state: State, ctx: ApolloContext, **kwargs):
        ...
        try:
            from apollo.utils.image_hash import dhash_hex, hamming_distance_hex
