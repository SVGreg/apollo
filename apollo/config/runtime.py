...
def read_ipc_port() -> int | None:
    """Read the active APOLLO IPC port from environment, state files, or local server API."""
    env_port = os.getenv(ENV_APOLLO_IPC_PORT)
    if env_port and env_port.strip().isdigit():
        return int(env_port.strip())

    candidate_paths = [get_temp_dir() / "apollo-ipc-port", get_app_dir() / IPC_PORT_FILENAME]
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={"User-Agent": "Apollo-IPC-Discovery"},
        )
...
def write_ipc_port(port: int) -> Path:
    ...
    os.environ[ENV_APOLLO_IPC_PORT] = str(port)
    port_file = get_ipc_port_file()
    all_targets = [
        port_file,
        get_temp_dir() / "apollo-ipc-port",
...
def clear_ipc_port() -> None:
    ...
    os.environ.pop(ENV_APOLLO_IPC_PORT, None)
    all_targets = [
        get_ipc_port_file(),
        get_temp_dir() / "apollo-ipc-port",
        get_app_dir() / IPC_PORT_FILENAME,
