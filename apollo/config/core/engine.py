...
from apollo.config.core.interpolator import interpolate_config_value
...
class CascadingConfigEngine:
    ...
    def load_raw_config(
        ...
        # Tier 1: Global user configuration ($CONFIG_DIR/apollo.jsonc)
        global_config_path = platform.paths.config_dir / "apollo.jsonc"
        ...
        # Tier 2: Workspace project configuration (config/apollo.jsonc)
        ws_config_path = self.workspace_root / "config" / "apollo.jsonc"
        if not ws_config_path.exists():
            ws_config_path = self.workspace_root / "apollo.jsonc"
