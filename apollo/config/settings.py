...
from apollo.config.paths import (
    GLOBAL_APP_DIR,
...
class Settings(BaseSettings):
    """Centralized APOLLO runtime settings loaded from environment and .env files."""
    ...
    APOLLO_HIERARCHY_BACKEND: str = Field(default="auto")
    # Whether a task may install / upgrade the Accessibility Helper APK on a device
    # it holds. False = only attach to a helper installed by `apollo helper install`.
    APOLLO_HELPER_AUTO_INSTALL: bool = Field(default=True)

    # Execution Defaults
    PROJECT_NAME: str | None = None
    APOLLO_DEFAULT_PROFILE: str = Field(default=DEFAULT_PROFILE)
    APOLLO_DEFAULT_MODEL: str = Field(default=DEFAULT_MODEL)

    # Explorer Tool Settings (the tier itself is configured in apollo.jsonc or
    # via APOLLO_EXPLORER_VERSION; see apollo.config.agent.ExplorerConfig)
    ...
    def get_api_key(self, provider: str) -> SecretStr | None:
        ...
        """
        from apollo.llm.google.provider import is_google_family_provider
