...
from apollo.config.paths import ROOT_DIR, get_config_path
...
#: values from ``apollo.jsonc``. ``final`` mirrors the factory layering.
...
class ExplorerConfig(BaseModel):
    ...
    def resolve(
        ...
        env_v = os.getenv("APOLLO_EXPLORER_VERSION", "").strip().lower()
...
class MemoryConfig(BaseModel):
    ...
    policies: dict[str, dict[str, Any]] = Field(
        ...
        description=(
            ...
            " (apollo.memory.context_policy); keys are agent names, values"
...
class AgentGlobalConfig(BaseModel):
    ...
    def synchronize_profiles_and_components(cls, data: Any) -> Any:
        ...
        pv_env = _parse_bool("APOLLO_PLANNER_VALIDATION_ENABLED")
        ...
        out_env = _parse_bool("APOLLO_OUTPUTTER_ENABLED")
        if out_env is not None:
            data.setdefault("outputter", {})["enabled"] = out_env

        vid_rec_env = _parse_bool("APOLLO_VIDEO_RECORDING_ENABLED") or _parse_bool(
            "APOLLO_WITH_VIDEO_RECORDING_TOOLS"
        )
        ...
        vid_env = _parse_bool("APOLLO_VIDEO_LEDGER_ENABLED")
...
def resolve_explorer_version(
    ...
    Precedence: explicit argument, ``APOLLO_EXPLORER_VERSION`` environment
...
def load_agent_config(
    ...
    """Load and validate AgentGlobalConfig from apollo.jsonc or agent_config.json.
    ...
    if not resolved_path:
        for candidate in ("apollo.jsonc", "apollo.json", AGENT_CONFIG_FILENAME):
            try:
