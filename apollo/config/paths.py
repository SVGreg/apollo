...
from apollo.config.constants import (
    DATA_ENGINE_DB_FILENAME,
    ENV_ANTIGRAVITY_APP_DIR,
    ENV_APOLLO_APP_DIR,
    ENV_APOLLO_TRACES_DIR,
    ENV_APOLLO_USE_USER_DIR,
    IMAGES_DIRNAME,
    IPC_PORT_FILENAME,
    LS_ADDRESS_FILENAME,
    PAUSE_FILENAME,
...
from apollo.resources import get_bundled_config_path

# Project root directory (3 levels up from apollo/config/paths.py)
...
def is_source_checkout() -> bool:
    """Return whether ``ROOT_DIR`` is an Apollo source checkout.

    Installed wheels also contain an ``apollo`` directory, so the repository
    ...
    """
    return (ROOT_DIR / "pyproject.toml").is_file() and (ROOT_DIR / "apollo").is_dir()
...
def _use_user_app_dir() -> bool:
    ...
    return (
        ...
        or os.getenv(ENV_APOLLO_USE_USER_DIR) == "true"
        or bool(os.getenv(ENV_APOLLO_APP_DIR))
        or bool(os.getenv(ENV_ANTIGRAVITY_APP_DIR))
    )


def get_app_dir() -> Path:
    """Returns the central application directory for APOLLO configurations, DBs, and traces.

    Order of precedence:
    1. APOLLO_APP_DIR or ANTIGRAVITY_APP_DIR environment variable.
    2. Legacy ~/.gemini/jetski or ~/.apollo directory if it exists.
...
def get_default_traces_path() -> Path:
    ...
    """
    env_traces = os.getenv(ENV_APOLLO_TRACES_DIR)
    if env_traces:
...
def get_temp_dir(subfolder: str | None = None) -> Path:
    """Returns a unified temporary directory for APOLLO runtime operations (e.g., screenshots, recordings).
...
def get_data_engine_db_path() -> Path:
    """Returns the unified SQLite database path for APOLLO DataEngine."""
...
def get_server_info_file() -> Path:
    """Returns the location of the Apollo server metadata file."""
...
def get_config_path(filename: str, default_bundled_path: Path | None = None) -> Path:
    ...
    Resolution Order:
    1. Environment variable override: `APOLLO_<FILENAME_UPPER>`
    ...
    """
    env_var = f"APOLLO_{filename.upper().replace('.', '_').replace('-', '_')}"
