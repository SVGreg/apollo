...
from apollo.platform.base import IPlatform, IPlatformPaths, IPlatformProcess, OSType
...
class LinuxPlatformPaths(IPlatformPaths):
    ...
    def config_dir(self) -> Path:
        ...
        if xdg_config:
            p = Path(xdg_config) / "apollo"
        else:
            p = self._home / ".config" / "apollo"
    ...
    def data_dir(self) -> Path:
        ...
        env_dir = os.getenv("APOLLO_APP_DIR") or os.getenv("ANTIGRAVITY_APP_DIR")
        if env_dir:
            p = Path(env_dir)
            p.mkdir(parents=True, exist_ok=True)
            return p
        ...
        if xdg_data:
            p = Path(xdg_data) / "apollo"
            p.mkdir(parents=True, exist_ok=True)
            return p

        # Check legacy ~/.gemini/jetski or ~/.apollo directory if it exists
        legacy_jetski = self._home / ".gemini" / "jetski"
        if legacy_jetski.exists():
            return legacy_jetski

        legacy_apollo = self._home / ".apollo"
        if legacy_apollo.exists():
            return legacy_apollo

        p = self._home / ".local" / "share" / "apollo"
        p.mkdir(parents=True, exist_ok=True)
        return p
    ...
    def cache_dir(self) -> Path:
        ...
        if xdg_cache:
            p = Path(xdg_cache) / "apollo"
        else:
            p = self._home / ".cache" / "apollo"
        p.mkdir(parents=True, exist_ok=True)
        return p
    ...
    def temp_dir(self, subfolder: str | None = None) -> Path:
        base = Path(tempfile.gettempdir()) / "apollo"
