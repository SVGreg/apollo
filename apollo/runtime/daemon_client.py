...
"""Client utilities for communicating with and auto-spawning the Apollo Daemon."""
...
DEFAULT_DAEMON_HOST = os.environ.get("APOLLO_DAEMON_HOST", "127.0.0.1")
DEFAULT_DAEMON_PORT = int(os.environ.get("APOLLO_DAEMON_PORT", "8000"))


def is_standalone_forced() -> bool:
    """Return True if standalone/embedded execution is explicitly configured."""
    return os.environ.get("APOLLO_STANDALONE", "").lower() in ("1", "true", "yes")
...
def is_daemon_running(
    ...
    """Fast probe to check if the Apollo Daemon HTTP server is responding."""
    url = f"http://{host}:{port}/api/system/readiness"
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Apollo-Daemon-Probe"})
        ...
        try:
            ...
            req = urllib.request.Request(root_url, headers={"User-Agent": "Apollo-Daemon-Probe"})
            with urllib.request.urlopen(req, timeout=timeout) as response:
...
def is_apollo_daemon(
    host: str = DEFAULT_DAEMON_HOST,
    port: int = DEFAULT_DAEMON_PORT,
    timeout: float = 0.5,
) -> bool:
    """Identity probe: is the process on ``host:port`` the Apollo daemon?
...
def spawn_daemon(
    ...
    """Silently spawn the Apollo Daemon server in the background as a detached process.

    The server is always launched via ``sys.executable -m`` (never through the
    ``apollo``/``apollo-admin`` console-script shims) so the long-lived process
    chain never holds a lock on ``Scripts/apollo.exe`` — on Windows, a resident
    ...
    # default for `apollo restart`, so silent DEVNULL would hide crashes).
...
def ensure_daemon_running(
    ...
    """Ensure the Apollo Daemon is running, auto-spawning it if necessary.
    ...
    *,
    ...
    try:
        ...
        req = urllib.request.Request(
            url,
            data=data,
            headers={"Content-Type": "application/json", "User-Agent": "Apollo-Client"},
            method="POST",
        )
        with urllib.request.urlopen(req, timeout=timeout) as response:
...
def stop_task_on_daemon(
    ...
    *,
    ...
    """Request the running Apollo Daemon to stop a task by session ID."""
...
def get_daemon_session(
    ...
    *,
    ...
    """Fetch details of a single session from the Apollo Daemon."""
    url = f"{base_url or f'http://{DEFAULT_DAEMON_HOST}:{DEFAULT_DAEMON_PORT}'}/api/sessions/{session_id}"
    try:
        req = urllib.request.Request(
            url,
            headers={"User-Agent": "Apollo-Client"},
            method="GET",
        )
        with urllib.request.urlopen(req, timeout=timeout) as response:
            if response.status == 200:
...
def get_daemon_status(
    *,
    ...
    """Fetch global scheduler and device status from the Apollo Daemon."""
