...
"""Lifecycle of the Apollo Accessibility Helper APK on each Android device.

The helper (``packages/apollo-accessibility-helper``) is an accessibility
...
* **Provision** (persistent on the device, survives reboots): the APK is
  ...
  ``DeviceExecutionLock`` boundary, ``apollo helper install``,
...
* **Attach** (one process, one device, one session): an ``adb forward`` from a
  ...
  forwards for the same serial are reused so two Apollo processes share one
...
* **Token**: the loopback port is reachable by every app on the phone, so the
  ...
  Apollo temp directory so every Apollo process on the machine presents the
...
holder: uiautomator2's server (Apollo's own fallback, or one left behind by a
...
from apollo.config.constants import ENV_APOLLO_HELPER_AUTO_INSTALL
...
PACKAGE_NAME = "com.apollo.helper"
SERVICE_NAME = f"{PACKAGE_NAME}/.ApolloAccessibilityService"
...
HELPER_PACKAGE_DIR = (
    Path(__file__).resolve().parent.parent.parent / "packages" / "apollo-accessibility-helper"
)
BUNDLED_APK_PATH = HELPER_PACKAGE_DIR / "ApolloAccessibilityHelper.apk"
...
#: uiautomator2 (openatx), which Apollo itself uses as the fallback backend and
...
MANUAL_ENABLE_PATH = (
    ...
    "Apollo Accessibility Helper > turn it on"
)
...
def auto_install_allowed() -> bool:
    ...
    ``APOLLO_HELPER_AUTO_INSTALL`` (environment, then settings); default true.
    """
    raw = os.environ.get(ENV_APOLLO_HELPER_AUTO_INSTALL)
    if raw is None:
        try:
            from apollo.config import settings

            value = getattr(settings, "APOLLO_HELPER_AUTO_INSTALL", True)
            return (
                bool(value)
...
class HelperSession:
    ...
    #: Session token the helper expects in ``X-Apollo-Token``; None for pre-token helpers.
...
class AccessibilityHelperManager:
    ...
    def host_token(self) -> str:
        """The per-host secret every Apollo process presents to the helper.

        Created on first use (48 hex chars) in the Apollo temp directory with
    ...
    def stop_uiautomator2_server(self, serial: str, pids: list[int] | None = None) -> bool:
        ...
        which Apollo owns as its fallback backend. Returns whether anything was stopped.
        """
        if pids is None:
    ...
    def _provision_mutex(self, serial: str):
        """Serialise installs across Apollo processes sharing one device."""
                ...
                if installed is None:
                    return ProvisionResult(
                        ...
                        error=(
                            ...
                            f"{ENV_APOLLO_HELPER_AUTO_INSTALL}=false forbids installing it "
                            f"from a task. Install it once with: apollo helper install "
                            f"--serial {serial}"
                        ),
                    )
                logger.info(
                    ...
                    f"{ENV_APOLLO_HELPER_AUTO_INSTALL}=false."
                )
                action = "skipped"
                needs_install = False
            if needs_install:
                if bundled is None:
                    ...
                    # This is the one moment Apollo changes what is installed on the
                    # phone; say so in a way that survives in every log.
                    logger.warning(
                        f"Installing the Apollo accessibility helper v{bundled.version_name} "
                        ...
                        "stays installed until removed with `apollo helper uninstall`. Set "
                        f"{ENV_APOLLO_HELPER_AUTO_INSTALL}=false to install it by hand only."
                    )
                    emit(
            ...
            if info is None:
                ...
                if suppressed_by:
                    raise HelperUnavailable(
                        ...
                        "capability) or run with APOLLO_HIERARCHY_BACKEND=uiautomator."
                    )
                raise HelperUnavailable(
            ...
            if protocol < MIN_PROTOCOL_VERSION:
                ...
                if protocol < MIN_PROTOCOL_VERSION:
                    ...
                    raise HelperUnavailable(
                        ...
                        f"apollo helper install --serial {serial} --force"
                    )
