...
"""USB-scoped screen-awake policy shared by every local Apollo entrypoint."""
...
def _awake_enabled() -> bool:
    enabled = os.environ.get("APOLLO_KEEP_DEVICE_AWAKE", "true").strip().lower()
...
def _configure_usb_stay_awake(device_id: str) -> str | None:
    ...
    The policy is intentionally scoped to USB power rather than an Apollo
    ...
    """
    if not _awake_enabled() or os.environ.get("APOLLO_CLOUD_MODE") == "1":
        return None

    # Migration cleanup: previous Apollo versions used a persistent shell
...
def _discover_connected_device_ids() -> list[str]:
    """Return the explicitly targeted device, or the connected devices Apollo manages."""
    if not _awake_enabled() or os.environ.get("APOLLO_CLOUD_MODE") == "1":
        return []

    target = os.environ.get("APOLLO_DEVICE_ID") or os.environ.get("ADB_DEVICE_SERIAL")
    ...
    # Keep awake only the devices Apollo actually manages: serials claimed in
...
class ScreenAwakeService:
    ...
    def start(self, device_ids: Iterable[str] | None = None) -> dict[str, str | None]:
        ...
        if (
            _awake_enabled()
            and os.environ.get("APOLLO_CLOUD_MODE") != "1"
    ...
    def ensure_device(self, device_id: str) -> str | None:
        ...
        if (
            ...
            or os.environ.get("APOLLO_CLOUD_MODE") == "1"
    ...
    def _start_heartbeat(self, device_id: str) -> None:
        ...
        thread = threading.Thread(
            target=heartbeat,
            name=f"apollo-awake-heartbeat-{device_id}",
            daemon=True,
        )
    ...
    def _start_device_monitor(self) -> None:
        with self._lock:
            ...
            thread = threading.Thread(
                target=monitor,
                name="apollo-awake-device-monitor",
                daemon=True,
            )
