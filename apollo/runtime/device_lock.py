...
"""Cross-process FIFO execution queue and per-device mutex for Apollo devices."""
...
class DeviceBusyError(RuntimeError):
    """Raised when another Apollo process already owns a device."""


class ConcurrencyMode:
    """Supported concurrency strategies for Apollo task execution."""
...
class DeviceExecutionLock:
    ...
    Apollo enforces a single active task per mobile device at any time.
    Different devices can execute tasks concurrently unless a global concurrency
    limit (e.g. concurrency_mode="global" or APOLLO_MAX_CONCURRENT_TASKS=1) is configured.
    ...
    QUEUE_TICKET_ENV = "APOLLO_DEVICE_QUEUE_TICKET"
    LOCK_SCOPE_ENV = "APOLLO_ADB_ENDPOINT_ID"
    ...
    def __init__(
        ...
        self.path = lock_dir / f"apollo-device-{self.clean_device_id}.lock"
        # Shared FIFO wait queue directory across all processes
        self.queue_dir = lock_dir / "apollo-global-device.queue"
        ...
        self.session_id = (
            session_id or os.getenv("APOLLO_SESSION_ID") or os.getenv("APOLLO_CLOUD_SESSION_ID")
        )
        self.ingress = ingress or os.getenv("APOLLO_TASK_INGRESS") or "sdk"
    ...
    def _device_lock_path(
        ...
        return get_temp_dir("device-locks") / f"apollo-device-{clean_id}.lock"

    @classmethod
    def _global_lock_path(cls) -> Path:
        # Kept for backward compatibility and fallback detection
        return get_temp_dir("device-locks") / "apollo-global-device.lock"
    ...
    def _owner_is_alive(owner: DeviceLockOwner) -> bool:
        ...
        if owner.process_created_at <= 0:
            ...
            # protection; require an Apollo-looking process before trusting
            ...
            try:
                ...
                return any(k in name or k in cmdline for k in ("python", "apollo", "pytest"))
            except Exception:
                return False
    ...
    def _build_device_queue(
        ...
        *,
        ...
        idle_candidates = sorted(
            ...
            if not (lock_dir / f"apollo-device-{lock_id}.lock").exists()
        )
    ...
    def reserve(
        ...
        *,
        ...
        queue_dir = get_temp_dir("device-locks") / "apollo-global-device.queue"
        ...
        for _ in range(8):
            ...
            try:
                ...
                try:
                    ...
                    os.close(fd)
                return token
        raise DeviceBusyError("Could not reserve a position in the Apollo device queue.")
    ...
    def _claim_or_create_queue_ticket(self) -> Path:
        ...
        if self.queue_ticket:
            ...
            if path is None:
                raise DeviceBusyError(
                    "The Apollo queue reservation was cancelled or expired before execution."
                )
            ...
            if not self._atomic_write_json(path, payload):
                raise DeviceBusyError(
                    "The Apollo queue reservation could not be claimed (ticket rewrite failed)."
                )
            return path
        ...
        path = self._find_reserved_ticket()
        if path is None:
            raise DeviceBusyError("The Apollo device queue ticket could not be created.")
        return path
    ...
    def resolve_env_concurrency(cls) -> int:
        ...
        """
        env_mode = os.environ.get("APOLLO_CONCURRENCY_MODE", "").strip().lower()
        if env_mode in ("global", "serial", "1"):
            return 1
        if env_mode in ("per_device", "device", "parallel", "0"):
            return 0
        try:
            return int(os.environ.get("APOLLO_MAX_CONCURRENT_TASKS", 0))
    ...
    def acquire(
        ...
        *,
        ...
        """Wait in FIFO order and acquire exclusive access to the target Apollo device."""
        ...
        try:
            while True:
                if cancel_event is not None and cancel_event.is_set():
                    raise DeviceBusyError("Waiting for the Apollo device queue was cancelled.")
                self._remove_stale_queue_entries()

                if not self._queue_path.exists():
                    raise DeviceBusyError("The Apollo queue reservation was cancelled.")
                ...
                if timeout is not None and time.monotonic() - started_at >= timeout:
                    raise DeviceBusyError(
                        f"Timed out waiting for the Apollo device queue after {timeout:.1f}s."
                    )
                time.sleep(max(0.01, poll_interval))
    ...
    def get_active_owners(cls) -> dict[str, DeviceLockOwner]:
        ...
        for lock_path in sorted(lock_dir.glob("apollo-device-*.lock")):
        ...
        global_lock = lock_dir / "apollo-global-device.lock"
    ...
    def get_queued_tasks(cls, device_id: str | None = None) -> list[dict[str, Any]]:
        ...
        lock_dir = get_temp_dir("device-locks")
        queue_dir = lock_dir / "apollo-global-device.queue"
        if not queue_dir.exists():
            return []
    ...
    def has_owner_record(cls, device_id: str | None = None) -> bool:
        ...
        if device_id is not None:
            ...
            return (lock_dir / f"apollo-device-{clean_id}.lock").exists() or (
                lock_dir / "apollo-global-device.lock"
            ).exists()
        return (
            bool(list(lock_dir.glob("apollo-device-*.lock")))
            or (lock_dir / "apollo-global-device.lock").exists()
        )
    ...
    def is_active_owner(cls, expected: DeviceLockOwner, device_id: str | None = None) -> bool:
        ...
        path = lock_dir / f"apollo-device-{clean_id}.lock"
        if not path.exists():
            path = lock_dir / "apollo-global-device.lock"
    ...
    def _find_lock_by_pid(cls, pid: int) -> Path | None:
        ...
        if not lock_dir.exists():
            return None
        for path in sorted(lock_dir.glob("apollo-device-*.lock")):
            owner = cls._read_owner(path)
    ...
    def cleanup_stale_locks(cls, device_id: str | None = None) -> int:
        ...
        patterns = (
            [f"apollo-device-{cls._normalize_device_id(device_id)}.lock"]
            if device_id
            else ["apollo-device-*.lock", "apollo-global-device.lock", "*.lock"]
        )
