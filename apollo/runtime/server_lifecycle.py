...
"""Apollo Server Lifecycle and Process Management.

Provides cross-platform facilities to discover, inspect, terminate,
and restart running Apollo UI / backend server instances regardless
...
from apollo.config.paths import ROOT_DIR, get_server_info_file
...
def write_server_info(
    ...
    """Persist current Apollo server runtime metadata to well-known path."""
    ...
    try:
        ...
        logger.debug(f"Saved Apollo server info (PID: {data['pid']}, port: {port}) to {info_file}")
...
def clear_server_info(
    *,
    ...
    Scoped removal keeps ``apollo stop --port <unused>`` and a late shutdown
...
def request_graceful_shutdown(port: int, timeout: float = 1.5) -> bool:
    """Ask a locally managed Apollo server to run its shutdown lifecycle.
    ...
    request = urllib.request.Request(
        ...
        method="POST",
        headers={
            "User-Agent": "Apollo-Lifecycle-Client/1.0",
            "X-Apollo-Lifecycle-Token": token,
        },
    )
    try:
...
def find_server_pids(port: int = 8000) -> list[int]:
    """Find all process IDs associated with the Apollo server listening on `port`.
            ...
            for proc in psutil.process_iter(["pid", "cmdline"]):
                try:
                    ...
                    if "apps.admin_console.server" in cmdline or (
                        "apollo" in cmdline and "ui" in cmdline
                    ):
...
def get_server_status(port: int = 8000) -> dict[str, Any]:
    """Retrieve detailed status of the Apollo server on given port."""
...
def stop_server(
    ...
    """Gracefully terminate any running Apollo server on `port`.
    ...
    if not force:
        ...
        if graceful_requested:
            ...
            if not is_port_in_use(port) and not _any_pid_alive(pids):
                ...
                msg = f"Apollo server stopped gracefully{pid_str}. Port {port} is released."
                if cleaned_locks:
                    msg += f" Cleaned up {cleaned_locks} device lock(s)."
                return True, msg, pids

            logger.warning(
                f"Apollo server on port {port} did not exit within {timeout}s; "
    ...
    if port_free:
        ...
        msg = f"Apollo server stopped{pid_str}. Port {port} is released."
        if cleaned_locks:
            msg += f" Cleaned up {cleaned_locks} device lock(s)."
