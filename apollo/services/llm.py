...
"""Universal LLM Service for Apollo.
...
from apollo.llm.google import is_google_chat_model, is_google_provider
...
# Provider retry logs do not carry an Apollo trace id.  Keep the active model
...
def _get_current_data_engine():
    ...
    engine_module = sys.modules.get("apollo.data_engine.engine")
...
class _ProviderRetryTelemetryHandler(logging.Handler):
    """Turn provider SDK retry log records into structured Apollo telemetry."""
...
async def _run_with_recovery[T](
    ...
    *,
    ...
    Failure handling is decided per category (see apollo.llm.reliability):
