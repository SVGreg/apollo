...
between the actuator implementations (``apollo/mcp/actuators/``) and the agents that
...
2. Everything else is OPTIONAL, because every other prompt reference lives in a tool
   ...
   (``apollo/agents/prompt_assembly.py``) renders conditionally. An optional tool that
