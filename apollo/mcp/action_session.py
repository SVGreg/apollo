...
torn down from ``ApolloContext.__aexit__``.
...
The single owner loop also serializes calls, which enforces APOLLO's
...
class ActionSession:
    ...
    async def start(self) -> "ActionSession":
        if self._owner is not None:
            return self
        self._owner = asyncio.create_task(self._run(), name="apollo-action-session")
