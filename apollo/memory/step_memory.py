...
class StepLens:
    ...
    transition lens (:class:`~apollo.agents.flash.summarizer.VisualStepSummarizer`
    keeps it inlined as its ``_attempt`` for compatibility) and the chunk-level
    :class:`~apollo.memory.chunking.StepCapsuleLens`.
    """
...
class StepMemoryService:
    ...
    6. Detached tracing: every job runs inside
       :func:`~apollo.data_engine.trace.detached_trace`, so a lens's model
