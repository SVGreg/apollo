...
from apollo.llm.google import is_google_provider
from apollo.memory.step_memory import JobKey, StepLens, StepMemoryService
...
def _result_phrase(result: Any) -> str:
    ...
    from apollo.utils.task_tree import format_result_clean
...
def _visual_summary_marker() -> str:
    ...
    Imported lazily: ``context_compressor`` imports ``apollo.memory``, so a
...
class StepCapsuleLens(StepLens):
    ...
    saw it (:func:`apollo.memory.transcript.render_turn_transcript`). The
    ...
    async def render(self, key: JobKey, payload: dict[str, Any]) -> str | None:
        ...
        if self.has_fallback:
            from apollo.services.llm import with_fallback
...
class HistoryChunkManager:
    ...
    def _resolve_capsule_fallback_model(self, ctx: Any) -> str | None:
        ...
        try:
            ...
            if llm_cfg is None:
                from apollo.config.llm import get_default_llm_config
    ...
    def _resolve_label(self, seg_hash: str | None) -> str | None:
        ...
        try:
            ...
            from apollo.utils.plan_grammar import parse_plan, subgoal_hash
            from apollo.utils.task_tree import get_all_subgoal_aliases
