...
`apollo-client` is the remote Python SDK that runs on a developer machine. It
communicates with the Apollo daemon on a device host over HTTP and contains no
...
```powershell
uv add --editable C:\path\to\apollo\packages\apollo-client
```
...
The current Apollo daemon administration API does not include built-in remote
...
```powershell
ssh -L 8000:127.0.0.1:8000 user@apollo-host
```
...
client = ApolloClient("http://127.0.0.1:8000")
```
...
Version 0.1 supports these current Apollo daemon endpoints:
