...
The distribution is named `apollo-client`, while the Python import is
`apollo_client`. This prevents it from shadowing the full runtime's top-level
`apollo` package. The full runtime can depend on this package later and
re-export the client from `apollo` for compatibility.
