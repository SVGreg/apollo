...
"""Dependency-free data models for the Apollo remote API."""
...
class TaskHandle:
    """A task accepted by the remote Apollo scheduler."""
    ...
    def from_payload(cls, payload: Mapping[str, Any]) -> TaskHandle:
        ...
        if not task_id:
            from apollo_client.errors import ProtocolError
...
@dataclass(frozen=True, slots=True)
class TaskResult:
    """Current or terminal state of a remote Apollo task."""
...
@dataclass(frozen=True, slots=True)
class Device:
    """A device reported by the remote Apollo host."""
...
@dataclass(frozen=True, slots=True)
class Capabilities:
    """Features advertised by an Apollo host."""
