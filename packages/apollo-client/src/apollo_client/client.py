...
"""Asynchronous, remote-only Apollo client."""
...
from apollo_client.errors import NotFoundError, ProtocolError, TaskRejectedError, TaskTimeoutError
...
class ApolloClient:
    """Thin client for an Apollo daemon running on another host.

    This class never imports ADB, an Apollo agent, an LLM provider, or a
    ...
    def __init__(
        ...
        *,
        ...
        resolved_base_url = base_url or os.environ.get("APOLLO_BASE_URL")
        if not resolved_base_url:
            daemon_host = os.environ.get("APOLLO_DAEMON_HOST", "127.0.0.1")
            daemon_port = os.environ.get("APOLLO_DAEMON_PORT", "8000")
    ...
    def set_device(self, device_serial: str) -> ApolloClient:
        """Set the default remote device and return this client."""
        self._device_serial = device_serial
        return self

    def set_concurrency_mode(self, mode: str) -> ApolloClient:
    ...
    async def list_devices(self) -> tuple[Device, ...]:
        """List Android devices visible to the remote Apollo host."""
    ...
    async def submit(
        ...
        *,
        ...
        if status == "rejected" or (isinstance(tasks, list) and not tasks):
            detail = str(response.get("error") or "Apollo host rejected the task")
